...
# 
# 
#### CSBBSC Data Visualizations ----------------------------------------------
# Because Sean cleaned everything already, we can just start with where he left off
# which is all the stuff commented out above
load("~/Desktop/pbi/PICCBI/PICCBI_2021.RData")

# Condition Plot on poster
plot(sanitycheckplot)

# H1 judgements of other bodies
...
# I put a color filter over these to make some blue/red, but these grays can be changed out for any color

# this contains the other plots/analyses for this project
...
# * * * Covariates ####
# I have a cleaned & anonymized file on my computer
female_covariates <- read_csv("data/clean/female_covariates.csv")

# take the task data, connect it to the cleaned covariates
df_combined_1 <- left_join(self_q, raneffs, by = "subject")

analysis <- 
  df_combined_1 %>% 
  select(subject, selfcat, selfcont, edeq_sum, nfc_sum, bsqr_sum, bsi_sum, trial0)

c <- left_join(analysis, female_covariates, by = "subject") 
c2 <- subset(c, !is.na(soc_1)) # I have 2 more people compared to Sean, it's because of participant nx56atwwmpw912w, who revoked their consent, take them out

covariates_ms <- subset(c2, c2$subject != 'nx56atwwmpw912w') # I have 2 more people compared to Sean, it's because of participant nx56atwwmpw912w, who revoked their consent and somehow got in there twice, take them out
#yay! ok let's table everyone and make sure there are no duplicate subjects
check <- count_(covariates_ms$subject)
print(check)
# perfect

write_csv(covariates_ms, "data/clean/covariates_ms.csv")

# to see how the covariates were cleaned, check the covariate_cleanup file
# This way, the covariates should be joined to trial0 (the slope for each participant), to check if any covariates explain the individual differences
# spoilier alert, they do not!
