...
jsPsych.plugins['multiple-slider'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    html += ".jspsych-multiple-slider-statement { display:block; font-size: 0px; padding-top: 0px; margin-bottom:0px; }"+
      ...
      ".jspsych-multiple-slider-opts:before { content: ''; position:relative; top:4px; /*left:9.5%;*/ display:block; background-color:#efefef; height:4px; width:100%; }"+
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      for(var j=0; j < question.labels.length; j++){
        ...
        html += '<span style="text-align: center; font-size: 100%; color: black">'+question.labels[j]+'</span>'; // SD: change label colour (gray --> white)
        html += '</div>'
      }
      html += '</div>';
